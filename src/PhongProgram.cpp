...
FragVertex PhongProgram::vertexShader(const Vertex &vertex) const {
    return {getProjectionMatrix() * getViewMatrix() * getModelMatrix() *
            ...
            glm::normalize(getNormalMatrix() * vertex.normal), vertex.texture,
            glm::vec4(0.0, 0.0, 0.0, 0.0)};
}

Pixel PhongProgram::fragmentShader(const FragVertex &vertex,
    ...
    float specular =
        glm::pow(glm::max(glm::dot(normal, halfVector), 0.0f), material.shine) *
        energyConservation;
    glm::vec3 specularColor = material.specular * specular;
    glm::vec3 color =
        glm::clamp(diffuseColor + ambientColor + specularColor, 0.0f, 1.0f);
