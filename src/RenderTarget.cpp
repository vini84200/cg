...
RenderTarget::RenderTarget() {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(texture_coords),
...
void RenderTarget::init(RenderTargetConfig config) {
    ...
    glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, currentConfig.width,
...
void RenderTarget::render() {
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, currentConfig.width,
                    currentConfig.height, GL_RGBA, GL_UNSIGNED_BYTE,
                    pixels.data());
...
glm::mat4 RenderTarget::getViewportMatrix() const {
    return glm::mat4(
        ...
        glm::vec4(currentConfig.width / 2.0f,
                  currentConfig.height / 2.0f, 0.0f, 1.0f));
}

void RenderTarget::renderImGui() {
    ImGui::Text("Size: %d x %d", currentConfig.width,
                currentConfig.height);
    ...
    if (ImGui::TreeNode("Depth Buffer")) {
        ...
        ImGui::SliderFloat("Start", &currentConfig.depthBufferStart,
                           -1.0f, 1.0f);
        ImGui::SliderFloat("End", &currentConfig.depthBufferEnd,
                           -1.0f, 1.0f);

        // Render the depth buffer into the window
        for (auto pixel : depthPixels) {
            depthPixels_.push_back(
                (pixel - currentConfig.depthBufferStart)
                / (currentConfig.depthBufferEnd
                   - currentConfig.depthBufferStart));
        }

        // Render the depth buffer into a texture
        glBindTexture(GL_TEXTURE_2D, depth_texture);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, currentConfig.width,
                        currentConfig.height, GL_RED, GL_FLOAT,
                        depthPixels_.data());
        glBindTexture(GL_TEXTURE_2D, 0);
...
int RenderTarget::getWidth() const { return currentConfig.width; }

int RenderTarget::getHeight() const { return currentConfig.height; }
