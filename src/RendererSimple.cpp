...
RendererSimple::RendererSimple() {
    ...
    lightModelVSLocation = glGetSubroutineUniformLocation(program, GL_VERTEX_SHADER, "lightModelVS");

    if (lightModelVSLocation == -1) {
        printf("Error: Can't find LightModel uniform location in vertex shader\n");
    }

    shaderSubroutineIndicesVS[GOURAD_AD] = glGetSubroutineIndex(program, GL_VERTEX_SHADER, "gouroudAD");
    shaderSubroutineIndicesVS[GOURAD_SPEC] = glGetSubroutineIndex(program, GL_VERTEX_SHADER, "gouroudADS");
    shaderSubroutineIndicesVS[PHONG] = glGetSubroutineIndex(program, GL_VERTEX_SHADER, "phong");
    shaderSubroutineIndicesVS[NO_SHADE] = glGetSubroutineIndex(program, GL_VERTEX_SHADER, "noShade");

    lightModelFSLocation = glGetSubroutineUniformLocation(program, GL_FRAGMENT_SHADER, "LightModelFS");
    shaderSubroutineIndicesFS[GOURAD_AD] = glGetSubroutineIndex(program, GL_FRAGMENT_SHADER, "gouroudAD");
    shaderSubroutineIndicesFS[GOURAD_SPEC] = glGetSubroutineIndex(program, GL_FRAGMENT_SHADER, "gouroudADS");
    shaderSubroutineIndicesFS[PHONG] = glGetSubroutineIndex(program, GL_FRAGMENT_SHADER, "phong");
    shaderSubroutineIndicesFS[NO_SHADE] = glGetSubroutineIndex(program, GL_FRAGMENT_SHADER, "noShade");
...
void RendererSimple::render(Scene *scene, Camera *camera) {
    ...
    glUniform3fv( glGetUniformLocation( program, "viewPos" ), 1, &camera->getPos()[0] );
...
//        glGetIntegerv(GL_ACTIVE_SUBROUTINE_UNIFORM_LOCATIONS, &subroutineLocations);
//        assert(subroutineLocations != -1);
//        assert(subroutineLocations >= lightModelVSLocation);


        GLuint subroutineIndicesVS[subroutineLocations];
        GLuint subroutineIndicesFS[subroutineLocations];

        subroutineIndicesVS[lightModelVSLocation] = shaderSubroutineIndicesVS[shaderSubroutine];
        subroutineIndicesFS[lightModelFSLocation] = shaderSubroutineIndicesFS[shaderSubroutine];

        assert(shaderSubroutineIndicesVS[shaderSubroutine] != -1);
        assert(lightModelVSLocation == 0);
        glUniformSubroutinesuiv(GL_VERTEX_SHADER, 1, &shaderSubroutineIndicesVS[shaderSubroutine]);

        assert(shaderSubroutineIndicesFS[shaderSubroutine] != -1);
        assert(lightModelFSLocation == 0);
        glUniformSubroutinesuiv(GL_FRAGMENT_SHADER, 1, &shaderSubroutineIndicesFS[shaderSubroutine]);
...
void RendererSimple::renderImGui() {
    ...
    ImGui::Text("Shader Subroutine: %s", shaderSubroutineName[shaderSubroutine].c_str());
