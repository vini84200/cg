...
void RendererCloseToGlWithRasterizer::render(Scene *scene,
                                             Camera *camera) {
    ...
    renderTarget.render();
    //    FrameImage(renderTarget.getPixelData(),
    //    renderTarget.getWidth(), renderTarget.getHeight(), 0, 1);
    FrameMarkEnd("RendererCloseToGlWithRasterizer::render");
}

void RendererCloseToGlWithRasterizer::renderObject(
    ...
    if (colorOverride_) {
        Material m = Material(newColor, glm::vec3(0.0f),
                              glm::vec3(0.0f), 10.0f);
        ...
        if (m == nullptr) {
            m = new Material(glm::vec3(1.0f), glm::vec3(0.0f),
                             glm::vec3(0.0f), 10.0f);
        }
...
void RendererCloseToGlWithRasterizer::onResize(int width,
                                               int height) {
