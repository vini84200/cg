...
glm::mat4 Camera::getViewMatrix() {
    ...
    glm::mat4 view(right.x, up.x, -forward.x, 0, right.y, up.y,
                   -forward.y, 0, right.z, up.z, -forward.z, 0,
                   -glm::dot(glm::vec3(right), glm::vec3(pos)),
                   -glm::dot(glm::vec3(up), glm::vec3(pos)),
                   glm::dot(glm::vec3(f), glm::vec3(pos)), 1);
    //    glm::mat4 obj = glm::lookAt(glm::vec3(pos), glm::vec3(pos +
    //    forward), glm::vec3(0, 1, 0));
    //
    //    assert(glm::all(glm::epsilonEqual(view[0], obj[0],
    //    0.001f))); assert(glm::all(glm::epsilonEqual(view[1],
    //    obj[1], 0.001f)));
    //    assert(glm::all(glm::epsilonEqual(view[2], obj[2],
    //    0.001f))); assert(glm::all(glm::epsilonEqual(view[3],
    //    obj[3], 0.001f)));
...
glm::mat4 Camera::getProjectionMatrix() {
    ...
    //    near, far);
    glm::mat4 proj
        = glm::mat4(1.0f / (aspectRatio * glm::tan(radianFov / 2.0f)),
                    0, 0, 0, 0, 1.0f / glm::tan(radianFov / 2.0f), 0,
                    0, 0, 0, -(far + near) / (far - near), -1, 0, 0,
                    -(2.0f * far * near) / (far - near), 0);

    //    glm::mat4 obj = glm::perspective(glm::radians(fov),
    //    aspectRatio, near, far);
    //
    //    assert(glm::all(glm::epsilonEqual(proj[0], obj[0],
    //    0.001f))); assert(glm::all(glm::epsilonEqual(proj[1],
    //    obj[1], 0.001f)));
    //    assert(glm::all(glm::epsilonEqual(proj[2], obj[2],
    //    0.001f))); assert(glm::all(glm::epsilonEqual(proj[3],
    //    obj[3], 0.001f)));
...
void Camera::renderImGui() {
    ...
    if (ImGui::TreeNode("View Matrix")) {
        ...
        ImGui::Text(" %f  %f  %f  %f", view[0][0], view[1][0],
                    view[2][0], view[3][0]);
        ImGui::Text(" %f  %f  %f  %f", view[0][1], view[1][1],
                    view[2][1], view[3][1]);
        ImGui::Text(" %f  %f  %f  %f", view[0][2], view[1][2],
                    view[2][2], view[3][2]);
        ImGui::Text(" %f  %f  %f  %f", view[0][3], view[1][3],
                    view[2][3], view[3][3]);
    ...
    if (ImGui::TreeNode("Projection Matrix")) {
        ...
        ImGui::Text(" %f  %f  %f  %f", projection[0][0],
                    projection[1][0], projection[2][0],
                    projection[3][0]);
        ImGui::Text(" %f  %f  %f  %f", projection[0][1],
                    projection[1][1], projection[2][1],
                    projection[3][1]);
        ImGui::Text(" %f  %f  %f  %f", projection[0][2],
                    projection[1][2], projection[2][2],
                    projection[3][2]);
        ImGui::Text(" %f  %f  %f  %f", projection[0][3],
                    projection[1][3], projection[2][3],
                    projection[3][3]);
...
void Camera::update(float dt) {
    glm::vec3 u = glm::normalize(
        glm::cross(glm::vec3(forward), glm::vec3(0, 1, 0)));
    glm::vec3 v = glm::normalize(glm::cross(u, glm::vec3(forward)));
    glm::vec3 w = glm::normalize(glm::vec3(forward));

    glm::mat4 toWorld = glm::mat4(u.x, u.y, u.z, 0, v.x, v.y, v.z, 0,
...
void Camera::onMouseMoveDelta(double dx, double dy) {
    if (is_mouse_pressed_) {
        ...
        glm::vec3 pro_lado   = glm::normalize(
            glm::cross(glm::vec3(forward), glm::vec3(0, 1, 0)));
        glm::vec3 pra_cima = glm::normalize(
            glm::cross(pro_lado, glm::vec3(forward)));

        if (is_orbital_) {

        } else {
            float speed = speed_up_ ? 0.001f : 0.0005f;
            forward     = glm::rotate(glm::mat4(1), (float)dx * speed,
                                      pra_cima)
                      * forward;
            forward = glm::rotate(glm::mat4(1), (float)dy * speed,
                                  pro_lado)
                      * forward;
        }
    }
...
glm::vec4 Camera::getCCSz() const { return glm::normalize(forward); }
...
glm::vec4 Camera::getCCSx() const {
    glm::vec3 pro_lado = glm::normalize(
        glm::cross(glm::vec3(forward), glm::vec3(0, 1, 0)));
    glm::vec3 pra_cima
        = glm::normalize(glm::cross(pro_lado, glm::vec3(forward)));
    return glm::normalize(glm::vec4(pra_cima, 0));
}

const glm::vec4 &Camera::getPos() const { return pos; }

void Camera::setPos(const glm::vec4 &pos) { Camera::pos = pos; }
