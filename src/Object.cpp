...
GLuint *Object::getVaOs() { return VAOs; }

GLuint *Object::getBuffers(int vao) { return buffers[vao]; }

GLuint Object::getNumVertices() const { return numVertices; }

void Object::setNumVertices(GLuint numVertices) {
    Object::numVertices = numVertices;
}

IndicesType Object::getIndecesType() const { return indicesType; }
...
glm::mat4 Object::getTransformMatrix() { return glm::mat4(); }
...
void Object::updateCameraVAO() {
    ...
    glBindBuffer(
        GL_ARRAY_BUFFER,
        getBuffers(VAO_IDs::CameraSpace)[Buffer_IDs::PositionBuffer]);
    glBufferSubData(GL_ARRAY_BUFFER, 0,
                    numVertices * 4 * sizeof(float), positions);

    glBindBuffer(
        GL_ARRAY_BUFFER,
        getBuffers(VAO_IDs::CameraSpace)[Buffer_IDs::NormalsBuffer]);
    glBufferSubData(GL_ARRAY_BUFFER, 0,
                    numVertices * 3 * sizeof(float), normals);

    glBindBuffer(
        GL_ARRAY_BUFFER,
        getBuffers(
            VAO_IDs::CameraSpace)[Buffer_IDs::TextureCoordBuffer]);
    glBufferSubData(GL_ARRAY_BUFFER, 0,
                    numVertices * 2 * sizeof(float), textures);
...
void Object::initVAO() {
    ...
    glBindBuffer(GL_ARRAY_BUFFER,
                 getBuffers(CameraSpace)[PositionBuffer]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(positions), positions,
                 GL_DYNAMIC_DRAW);
    glVertexAttribPointer(vPosition, 4, GL_FLOAT, GL_FALSE, 0,
                          BUFFER_OFFSET(0));
    ...
    // NORMALS
    glBindBuffer(GL_ARRAY_BUFFER,
                 getBuffers(CameraSpace)[NormalsBuffer]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(normals), normals,
                 GL_DYNAMIC_DRAW);
    glVertexAttribPointer(vNormals, 3, GL_FLOAT, GL_FALSE,
                          sizeof(Vertex),
    ...
    // TEXTURES
    glBindBuffer(GL_ARRAY_BUFFER,
                 getBuffers(CameraSpace)[TextureCoordBuffer]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(textures), textures,
                 GL_DYNAMIC_DRAW);
    glVertexAttribPointer(vTextureCoord, 2, GL_FLOAT, GL_FALSE,
                          sizeof(Vertex),
                          BUFFER_OFFSET(sizeof(glm::vec3) * 2));
...
Object::Object() { transformMatrix = glm::identity<glm::mat4>(); }

void Object::updateCameraVertices(const glm::mat4 &projMatrix,
                                  const glm::mat4 &viewMatrix) {
    const glm::mat4 &modelViewMatrix = viewMatrix * transformMatrix;
    const glm::mat4 &modelViewProjMatrix
        = projMatrix * modelViewMatrix;
    if (lastProjViewMatrix == modelViewProjMatrix
        && numVisibleVertices > 0) {
    ...
    const glm::mat4 &normalMatrixIn4
    ...
    for (int tri = 0; tri < numTriangles; tri++) {
        // If we are keeping the triangle then we increase the number
        // of vertices to keep Otherwise the triangle will be
        ...
        for (int i = 0; i < 3; i++) {
            ...
            cameraVertices[newIndex].position
                = modelViewProjMatrix
                  * originalVertices[index].position;

            if (cameraVertices[newIndex].position.w <= 0
                || std::isnan(cameraVertices[newIndex].position.w)) {
                // If the vertex is behind the camera or is invalid we
                // don't keep the triangle
                keep = false;
                break;
            }

            cameraVertices[newIndex].position
                /= cameraVertices[newIndex].position.w;

            if (std::abs(cameraVertices[newIndex].position.x) > 1
                || std::abs(cameraVertices[newIndex].position.y) > 1
                || std::abs(cameraVertices[newIndex].position.z)
                       > 1) {
                // If the vertex is outside the screen we don't keep
                // the triangle
                keep = false;
                break;
            }

            cameraVertices[newIndex].normal
                = normalMatrix * originalVertices[index].normal;
            cameraVertices[newIndex].texture
                = originalVertices[index].texture;
        }
    }
